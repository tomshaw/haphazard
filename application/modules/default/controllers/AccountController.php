...
class AccountController extends Zend_Rest_Controller
{
	...
	public function putAction()
	{
		...
		if ($request->isPut()) {
			...
			if (sizeof($errors)) {
				$response->setHttpResponseCode(503)->appendBody(Zend_Json::encode(array('errors'=>$errors)));
				return;
			}
			...
			$response->setHttpResponseCode(200)->setBody(Zend_Json::encode(array('success'=>array($message))));
		}
	}
	...
	public function verifyAction()
	{
		...
		$row->setFromArray(array('email'=>(string)$params['email']));
		...
		while (($avatar_name = @readdir($dir)) != false) {
			if ($avatar_name != '.' && $avatar_name != '..') {
				if (preg_match('/(\.gif$|\.png$|\.jpg|\.jpeg)$/is', $avatar_name)) {
					$available_avatars[$row][$col] = array('file' => $avatar_name, 'name' => ucfirst(str_replace("_", " ", preg_replace('/^(.*)\..*$/', '\1', $avatar_name))), 'path' => './img/avatars/' . $category . '/' . $avatar_name, 'dbname' => $category . '/' . $avatar_name);
					$col++;
					if ($col == 6) {
						$row++;
						$col = 0;
