				...
				if ($ipdata) { // A row in the database is matching this ip.
					if($row->interval) { // Survey allows interval voting.
						if ($ipdata->created <= time()) { // If time has passed add time + interval again.
							$pollsip->updateEntry(time() + $row->interval, array('poll_id' => $pollId,'ip' => $ip));
						} else {
					...
					$pollsip->insertEntry(array('poll_id' => $pollId, 'ip' => $ip, 'created' => $time));
				}
			} else {
...
// 			$rimages = rand(0, (count($files) - 1));
			...
			$classes = array('progress-info','progress-success','progress-warning','progress-danger');
			shuffle($classes);
