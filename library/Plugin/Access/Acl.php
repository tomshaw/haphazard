<?php
/**
 * Base Application ACL - (A)ccess (C)ontrol (L)ist
 * 
 ...
 */
class Plugin_Access_Acl extends Zend_Controller_Plugin_Abstract
{
	/**
	 * A static array of system roles. The entire system reads from this array. A public static
	 * helper method is used to access system roles outside of the scope of this object.
	 *
	...
	public function getRole()
	{
		...
		throw new Zend_Exception('Given ACL role is out of range. Check your identity in the database.');
	}
	...
	/**
	 * Returns the Zend_Auth numeric identity. Assumes persistence layers return in
	 * object mode and a column called identity contains an integer representing the
	 * users identity/user level.
	 *
	...
	public function getIdentity()
	{
		...
		if (!$zendAuth instanceof Zend_Auth) {
			return false;
		}
		...
		if ($this->getAuth()->hasIdentity()) {
			if (isset($this->getAuth()->getIdentity()->identity)) {
	 ...
	 * ACL Privileges.
	 *
	 * $this->getAcl()->allow('anonymous', 'profiles', 'hello'); // ex. profiles|controller hello|action
	 *
	 * @return Plugin_Access_Acl/Zend_Controller_Plugin_Abstract Fluent.
	 */
	...
	{
		if ($this->hasRole()) {
			return new ArrayObject(self::$_roles);
		}
